...
void AudioLerpEffect::process_event( clap_event_header_t const* hdr ) {
...
uint32_t AudioLerpEffect::note_ports_count( bool is_input ) {
  SFG_LOG_TRACE( host_, host_log_, "[{:s}] [{:p}] enter( is_input={} )", __FUNCTION__, static_cast< void* >( this ), is_input );
  return 0;
}

bool AudioLerpEffect::note_ports_get( uint32_t index, bool is_input, clap_note_port_info_t* out_info ) {
  SFG_LOG_TRACE( host_,
                 host_log_,
                 "[{:s}] [{:p}] enter( index={:d}, is_input={}, out_info={:p} )",
                 __FUNCTION__,
