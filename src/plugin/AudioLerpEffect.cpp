...
bool AudioLerpEffect::gui_create( std::string const& api, bool is_floating ) {
  ...
  if( !is_floating ) {
    ...
    WRAP_SDL_CALL_INST( SDL_SetStringProperty, windowCreateProps, SDL_PROP_WINDOW_CREATE_TITLE_STRING, "com.SFGrenade.AudioLerpEffect" );
  ...
  guiTimer_ = Timer::createNative( 1, std::bind( &AudioLerpEffect::guiTimerCallback, this ) );
  guiTimer_->start();
  return true;
}
